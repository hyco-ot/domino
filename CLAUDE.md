...
- The board's controls are **`−1 [+1] +2`**, centred as a unit: the two side buttons are the same size so the main one lands under the score instead of off to one side. `+2` writes **two separate rounds**, not one round worth two — two hands were played, and the log shows one row per hand. If the first of them wins the tanda, the second is refused by `addRound`'s `S.over >= 0` guard, so `+2` can never overshoot the target.
...
- **`[hidden]` loses to any rule that sets `display`.** `.linky`, `.wipe`, `.names2` and `.icon-btn` all define their own, so each needs an explicit `[hidden]{display:none}` or the `hidden` property does nothing. This has bitten four times; the last one shipped, because jsdom computes no styles and the test only checked the `hidden` property. `ocultos.mjs` now reads the CSS and cross-checks every `$('#x').hidden =` in the source, so run it whenever you hide something by property.
...
- The slide classes animate `> *`, but `.anim-fade` animates **the element itself** — it is used on single pieces (the mesa's tile, its mode caption, the tanda stepper) when the mode changes, not on a whole screen. `animar()` clears all four via `ANIMS`, so a new animation class has to be added there or it will never be removed.
