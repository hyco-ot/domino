...
- The tanda length is written **`5/3`** — total over hands-needed, which is how it is said at the table. `cuantaTxt()` is the only place that wording lives. **The first number is derived, not chosen:** it is how many hands can be played at most before somebody gets there, which is everyone else stalling one short — `totalTanda()` is `n * (paraGanar − 1) + 1`. With two that is the familiar `2N−1`; with three it is `3N−2`, so a 19/10 is a **28/10** at a trio. Only the label was ever wrong — `S.target` is `paraGanar()` and the game itself always counted right — but a trio played 27 hands under a sign saying 19. `cuantaTxt(v, n)` therefore takes the count, defaulting to the live table; **history has to pass `eqMano(r).length`**, or a stored trio read from a two-seat table is labelled with the wrong total. It is chosen with a stepper of **±2 between 3 and 41**: the step is what guarantees an odd number, and an even tanda can end in a tie.
...
- `ask()` hides its "no" button when `noLabel` is empty — an empty box sat there from 3.00 until 3.25, on the dialogs that only need an acknowledgement. That needed its own `[hidden]` rule, like everything else in this app.
...
- **`[hidden]` loses to any rule that sets `display`.** `.linky`, `.wipe`, `.names2`, `.icon-btn` and `.menu button` all define their own, so each needs an explicit `[hidden]{display:none}` or the `hidden` property does nothing. This has bitten **six** times. Both guards share a blind spot worth naming: **jsdom computes no styles**, so anything that is "in the DOM but not on screen" is invisible to the tests that boot the app. Any bug of that shape needs a guard that reads the CSS instead.

A third trap of the same family, found the same way: **`paso()` in the jsdom tests must `await` its callback.** With an `async` body, everything after the first `await` ran outside the `try`, so a failure there was an unhandled rejection nobody looked at — and the step had already printed `ok`. Three tests were passing without checking anything past their first `await`; fixing it immediately surfaced a real broken assertion in the backup test. A test that cannot fail is worse than no test.

`ocultos.mjs` guards it, and in 3.01 it had to be rebuilt: it only compared the element's own *classes*, so `.menu button` — a bare tag inside a container — walked straight past it and the new rotate-screen button shipped visible on desktops. It now parses the markup with jsdom and asks the DOM directly which display-setting selectors actually `matches()` the element, and whether any `[hidden]` rule matches it too. That is right in both directions; the class-name heuristic was wrong in both. (It also strips CSS comments first — the brace-splitting swallowed them into the selector, which then matched nothing.)
...
- **What gets shouted when a hand is won, in order: lisa → streak → who won.** The lisa takes everything, being the biggest thing that can happen; then `rachaDe(t)` — three in a row is when people start counting out loud, and `RACHA_MIN` is 3. The streak deliberately does not repeat the name: `bigReact` already paints it in the winner's colour, which is how the rest of the app says who. It counts backwards through the history while `r.ident === S.ident` and the same column keeps winning, so another table's hands cut it — a streak is recent by definition and the 400-hand window is plenty.
- **Leaving the table says how it stands.** "Volver al inicio" with hands played shows `comoQueda()`: total plus each column and its wins, the leader first. It reads `S.games`, which is exactly this tanda's tally and resets itself when the identity changes — no new state was needed. The name goes first and the number after, for two columns and three alike: any phrasing with a verb ("ganó" / "ganaron") breaks on "Nosotros", which is a pronoun. Same lesson as `cantoGanador()`.
- **The way back to a finished hand is an arrow, and a gesture.** Winning by points deliberately does *not* jump to the winner screen (`recount()` sets `winSeen`), so something has to say a hand ended. `#btn-rev` floats against the right edge, in `--ok`, and **swiping the board right does the same thing** — that is what the arrow is asking for. Tapping it slides it away first; the swipe does not animate, because the finger already made that movement and repeating it feels doubled. Both go through `verResultado(animando)`.
  - The swipe demands `dx > 60` **and** clearly horizontal (`|dx| > |dy| * 1.6`) — scrolling down the entry log must never open the result by accident.
  - **The two idle animations live on different elements on purpose**: an element has one `transform`, so the box's side-to-side (`vaiven`) and the glyph's `respira` would overwrite each other. The box moves, the icon inside breathes.
  - Both hang off the element itself rather than a class something toggles: while the arrow exists there is a hand to review, and that is exactly the stretch during which it should be asking for attention — so no repaint can cut it short or restart it.
  - In Clásico the arrow **stays** but holds still. It is the only way to the result; hiding it there would leave the screen with no door.
